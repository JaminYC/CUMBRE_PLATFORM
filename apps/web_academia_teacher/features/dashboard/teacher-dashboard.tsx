...
const FEATURES = [
  ...
  },
  {
    ...
    desc: "Crear lecciones, organizarlas por tema y preparar material de apoyo.",
    Icon: AuthoringIcon,
    color: "accent"
  },
  {
    ...
    label: "Seguimiento",
    desc: "Ver quién avanza, quién se está quedando y qué temas cuestan más.",
    Icon: TutorIcon,
    color: "neutral"
  },
] as const;
  ...
  if (mostrandoCarga) {
    return (
      <AppShell title="Panel docente" breadcrumbs={[{ label: "Panel docente" }]}>
        <LoadingPanel message="Cargando tu panel..." detail="Recuperando el avance de tus estudiantes." />
      </AppShell>
    );
  }
  ...
  return (
    ...
    >
      ...
      <section className="dash-features">
        <h3 className="dash-section-title">¿Qué quieres hacer?</h3>
      ...
      <section className="dash-students" id="learner-summaries">
        ...
        {overview.learnerSummaries.length ? (
          ...
          <EmptyState
            ...
            description="Cuando empiecen a estudiar, aquí vas a ver cómo le va a cada uno."
          />
        )}
      </section>
      ...
      {overview.conceptStruggles.length > 0 && (
        <section className="dash-concepts">
          <h3 className="dash-section-title">Temas que más cuestan</h3>
          <p className="dash-section-sub">Los que más se atoran entre tus estudiantes.</p>
